...
class ChatController extends Controller
{
   ...
   public function behaviors()
        ...
        public function actionIndex()
    {
        ...
        $_SESSION['username'] = yii::$app->user->identity->chatname;
        ...
        if ($_GET['action'] == "chatheartbeat") { $this->chatHeartbeat($dbh); } 
...
function sendChat($dbh) {
    ...
    $to = $_POST['to'];
   // $to = 'ndidate';
   //$query = MySession::find()->where(['=','user_id',$to])->all();
   $query = MySession::find()->joinWith(['user b'], true, 'INNER JOIN')->where(['b.chatname' => $to])->all();
    //print_r();exit;
    if(!empty($query)) {}
        else  exit(0);

    //$to = yii::$app->user->identity->name;
...
function chatHeartbeat($dbh) {
    ...
    $sql = "select * from chat where (chat.to = '".yii::$app->user->identity->chatname."' AND recd = 0) order by id ASC";
    $query = mysqli_query($dbh,$sql);
    $items = '';
